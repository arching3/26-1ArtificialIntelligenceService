...
def process_event_disclosures(
    ...
    result = EventProcessingResult(company_code=company_code)
    seen_keys = set()
    report_meta_by_receipt = _load_major_report_meta(dart, company_code, start, end)

    logger.info("[%s] 최근 %s일 수시공시 이벤트 조회 시작", company_code, lookback_days)
    for keyword in keywords:
        try:
            with contextlib.redirect_stdout(io.StringIO()):
                df = dart.event(company_code, keyword, start=start, end=end)
            if df is None or getattr(df, "empty", True):
                continue
            for _, series in df.iterrows():
                row = series.to_dict()
                event = _normalize_event(row, keyword, company_code)
                ...
                if receipt_no:
                    try:
                        ...
                        raw_text = _clean_event_text(raw_document)
                    except Exception as exc:
                        logger.warning("[%s] 이벤트 원문 다운로드 실패(%s): %s", company_code, receipt_no, exc)

                # LLM-based target detail extraction (방안 3)
                if raw_text and _should_extract_detail(event, raw_text):
                    detail = _extract_target_detail(raw_text)
                    if detail:
                        event["target_detail"] = detail
                        logger.info(
                            "[%s] target_detail 추출 완료(%s): %s",
                            company_code,
                            receipt_no,
                            detail[:100],
                        )
                ...
                result.documents.extend(_make_event_documents(event, raw_text))
                time.sleep(sleep_seconds)
        except Exception as exc:
            logger.warning("[%s] 이벤트 조회 실패(%s): %s", company_code, keyword, exc)
            result.failed_list.append({"event_label": keyword, "reason": str(exc)})
        finally:
            time.sleep(sleep_seconds)

    logger.info(
        "[%s] 수시공시 이벤트 처리 완료: events=%s, docs=%s, failed=%s",
        company_code,
        len(result.events),
