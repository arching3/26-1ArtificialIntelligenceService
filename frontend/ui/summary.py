...
# frontend 브랜치 변경: 요약 카드 HTML 렌더링을 streamlit_app.py에서 분리했습니다.
def render_summary(summary: dict[str, Any] | None, summary_status: str) -> None:
    ...
    if summary_status == "connecting":
        logger.warning("frontend_summary_render_connecting")
        st.info("데이터를 불러오지 못했습니다. 잠시 후 다시 시도해주세요.")
        return
    if summary_status == "indexing":
        logger.info("frontend_summary_render_indexing")
        st.info("공시 인덱싱이 완료되면 요약을 불러옵니다.")
        return
    if summary_status == "failed":
        logger.warning("frontend_summary_render_failed")
        st.info("공시 인덱싱 상태를 확인해 주세요.")
        return

    if not summary:
        logger.debug("frontend_summary_render_empty status=%s", summary_status)
    ...
    logger.debug("frontend_summary_render_ready expanded=%s field_count=%s", expanded, len(fields))
